...
(function() {
  ...
  dermis = {
    _stack: [],
    use: function(fn) {
      return dermis._stack.push(fn);
    },
    _runStack: function(mod, args, cb) {
      var idx, next;
      if (dermis._stack.length === 0) {
        return cb();
      }
      idx = -1;
      next = function() {
        var nex;
        nex = dermis._stack[++idx];
        if (nex == null) {
          return cb();
        }
    ...
    handleRoute: function(service) {
      ...
      return function(args) {
        ...
        return dermis.cache[service].when(function(mod) {
          return dermis._runStack(mod, args, function() {
            var _ref, _ref1;
    ...
    binding: {
      adapter: {
        subscribe: function(obj, kp, cb) {
          if (kp === "" && obj instanceof dermis.Collection) {
            obj.on("remove", cb);
            obj.on("add", cb);
          }
    ...
    model: function(o) {
      ...
      return Model = (function(_super) {
        ...
        function Model() {
          Model.__super__.constructor.call(this, o);
        }
    ...
    collection: function(o) {
      ...
      return Collection = (function(_super) {
        ...
        function Collection() {
          Collection.__super__.constructor.call(this, o);
        }
    ...
    bind: function(el, model) {
      rivets.configure({
        adapter: dermis.binding.adapter
      });
      return rivets.bind(el, model);
    }
  };

  dermis.Model = (function(_super) {
  ...
  dermis.Collection = (function(_super) {
    ...
    Collection.prototype.remove = function() {
      ...
      return this.emit('remove', items);
    };

    Collection.prototype.push = function() {
      var item, items, nitems, _i, _len,
        _this = this;
      items = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      for (_i = 0, _len = items.length; _i < _len; _i++) {
        ...
        if ((item != null ? item.on : void 0) != null) {
          (function(item) {
            var handle;
            handle = function() {
              if (__indexOf.call(_this.get('items'), item) >= 0) {
                return _this.emit('change:child', item);
              } else {
                return item.off('change', handle);
              }
            };
            return item.on('change', handle);
          })(item);
        }
      }
      nitems = this.get('items').concat(items);
      this.set('items', nitems);
      return this.emit('add', nitems);
    };
  ...
  mixer.extend(rivets.formatters, dermis.binding.formatters);

  mixer.extend(rivets.routines, dermis.binding.routines);

  mixer.extend(dermis, new mixer.emitter);

  mixer.listen(dermis);

  window.dermis = dermis;
